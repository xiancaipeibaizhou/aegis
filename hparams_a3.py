...
GROUPS = {
    ...
    "EXP_SEQ_3":  {"SEQ_LEN": 3,  "BATCH_SIZE": 64}, 
    "EXP_SEQ_5":  {"SEQ_LEN": 5,  "BATCH_SIZE": 64},
    "EXP_SEQ_10": {"SEQ_LEN": 10, "BATCH_SIZE": 32}, 
    "EXP_SEQ_20": {"SEQ_LEN": 20, "BATCH_SIZE": 16},

    # --- 维度 2: 模型容量与表征维度 (Capacity) ---
    ...
    # [注]: 已经彻底移除了 EXP_KER_* 组！
    # 因为 AEGIS 自带 ER-SKNet，能在 [1,3,5,7,9,11] 的全量池子中自适应计算感受野！
}

# ==========================================
# 2. 数据集特定最优 Base 锚点 (AEGIS 统一架构下的 SOTA 配置)
# ==========================================
DATASET_BEST = {
    ...
    "ids2017":     {"SEQ_LEN": 3,  "HIDDEN": 128, "HEADS": 8, "DROPEDGE_P": 0.0, "CL_LOSS_WEIGHT": 0.8, "LR": 0.002,  "MAX_CL_EDGES": 8192, "PATIENCE": 15},
    
    # ISCX2012: 老旧数据集，特征明文且单纯。降维防过拟合 (Hidden 64) 并禁止丢边 (Drop=0.0)；
    # 彻底告别以前的人工调参 (不再死磕 k=3)，由 AEGIS 在时序特征和图熵引导下自主寻优。
    "iscx2012":    {"SEQ_LEN": 30, "HIDDEN": 64,  "HEADS": 4, "DROPEDGE_P": 0.0, "CL_LOSS_WEIGHT": 0.5, "LR": 0.002,  "MAX_CL_EDGES": 8192, "PATIENCE": 15},
}
...
def resolve_hparams(group, env=None, dataset=None):
    ...
    h = {
        ...
        # ✨ 为 AEGIS 全局统一提供最大口径的卷积池！不再受限！
        "KERNELS": _parse_int_list(env.get("KERNELS", ""), fallback=[1, 3, 5, 7, 9, 11]),
        ...
        "PRETRAIN_EPOCHS": int(env.get("PRETRAIN_EPOCHS", "30")),
    }
